...
function transactionFields(body, { partial = false } = {}) {
  ...
  // merchant, date and amount can be changed but never blanked
  ...
  if (!partial || has('amount')) out.amount = num(body.amount, { field: 'amount', required: true });
...
module.exports = function transactionsRoutes({ store, plaid }) {
  const router = express.Router();

  router.get('/transactions', (_req, res) => res.json(store.read('transactions')));

  router.post('/transactions', route((req, res) => {
  ...
  router.put('/transactions/:id', route((req, res) => {
    ...
    store.update('transactions', list => {
      ...
      for (const [k, v] of Object.entries(fields)) {
        if (v === undefined || v === '') delete txn[k]; else txn[k] = v;
      }
