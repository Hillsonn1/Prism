      ...
      return `
        <tr>
          ...
          <td>
            ...
            ${t.card ? `<div class="txn-card-badge">${esc(t.card)}</div>` : ''}${t.pending ? `<div class="txn-pending-badge">pending</div>` : ''}
          </td>
...
async function plaidDisconnect(itemId, name) {
  if (!confirm(`Disconnect ${name}? Prism will stop syncing it.`)) return;
  const deleteTx = confirm(`Also delete the transactions already imported from ${name}?\n\nOK = delete them · Cancel = keep them`);
  try {
    await api('DELETE', `/api/plaid/items/${encodeURIComponent(itemId)}?deleteTransactions=${deleteTx ? 1 : 0}`);
    clearAllCaches();
    await loadAll();
    renderSettings();
